...
/**
 * Class redefines codes of mobile phone to our constant values. Class can give
 * to developers following information:
 * <ul>
 * <li/><i>defined platform name</i><br>
 * In case if device vendor not defined we'll recieve
 * <code>PLATFORM_NOT_DEFINED</code> like platform name. Same in this case
 * keyCodes will be setted like for Nokia and SE. It's done for work on
 * emulators, because on some of them it's impossible to define platform name.
...
public final class KeyCodeAdapter {
    ...
    /**
     * standart values for softkeys of different platforms used only in
     * predefining
     */
    ...
    /**
     * constructor. here is predefining of spesial keys and platform made
     */
    ...
    public static final KeyCodeAdapter getInstance(Canvas canvas) {
        instance = new KeyCodeAdapter(canvas);
        return instance;
    }

    /**
     * return platform keycode of left softkey if it's defined default value -6
     *
     * @return SOFTKEY_LEFT
     */
...
//                }  
    ...
    /**
     * Used to adopt key kode to predefined constances, which are platform
     * independent.
     * <p/>
     * You can use this method in any kind of canvas, but better at first time
     * to call
     * <code>getInstance()</code> method at the beginning of midlet work,
     * because initialisation takes time.
     * <p/>
     * Best variant for usage is calling
     * <code>adoptKeyCode()</code> to use
     * <code>keyPressed()</code> method in Canvas:
     * <pre>
     * protected void keyPressed(int keyCode) {
     *     keyCode = KeyCodeAdapter.getInstance().adoptKeyCode(keyCode);
     * }
     * </pre> and then you can use it:
     * <pre>
     * switch (keyCode) {
     ...
     *     break;
     * }</pre> or send this code to any other clesses.
     *
